...
local myPath = os.getenv("UG4_ROOT").."/apps/geometry_sampling"
...
--local v_alpha = 0.250000000000000   	 -- rate of infection
...
function ReactionS(s,e) return v_alpha*s*e end
function ReactionS_ds(s,e) return v_alpha*e end
function ReactionS_de(s,e) return v_alpha*s end
...
if (ARGS.limexNumStages<2) then
	...
	output_file = "simdata/simdata_step_"..step ..".csv"
	output_density = "density/density_step_"..step..".csv"
	...
	while (endTime == nil or ((time < endTime) and ((endTime-time)/maxStepSize > relPrecisionBound))) do
		...
		output_file_new = "simdata/simdata_step_"..step ..".csv"
		output_density_new = "density/density_step_"..step..".csv"
	...
	while keep_moving do
		...
		if file == nil then
			...
			-- else, move the file
			os.rename("./Solution_t"..file_ending..".vtu", "./solution/Solution_t"..file_ending..".vtu")
			out_i = out_i + 1
		io.close(file)
		end
	end
	...
	local associations = run_parser(common_config, "geometry_parser.config", "#")
	print("done")
	io.write("loading convert_values...")
	ug_load_script(common_scripts.."convert_values.lua")
	print("done")
