	...
	for cols_i=1, #cols do
		...
		for time=1, timesteps do
			...
			for subset=1, #vertex_subset_ass do
				...
				output[cols_i][time][subset] = grid_sum / #vertex_subset_ass[subset]
				-- calculate total population = normalized_fraction * area [km^2] * density [1/km^2]
				output[cols_i][time][subset] = output[cols_i][time][subset] * area_sizes[subset] * densities[time][subset][1]
			end	
		end
	end
	return output
