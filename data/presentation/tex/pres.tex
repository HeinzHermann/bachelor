		...
		\onslide<1->{Limits of the \textbf{ODE} model:
			\begin{enumerate}[$\bullet$]
				\item Only dependent on time\\
					$\rightarrow$ no "pacial or structural "awareness" of the model
				\item Different regions need to either be modeled individually or clustered together\\
			\end{enumerate}
		}
		\vspace{0.5cm}
		\onslide<2>{
			...
			\begin{enumerate}[$\bullet$]
				\item Other parameters can be factored into the modelling process (space, democgraphic, etc.)
...
\begin{frame}
	...
	\begin{equation*} \label{eq1}
		\begin{split}
			\frac{d\rho S}{dt} + \nabla[-\rho D \nabla G] & = -\rho \alpha SE \\
			\frac{d\rho E}{dt} + \nabla[-\rho D \nabla A] & = \rho(\alpha SE-\frac{1}{qq}E)\\
			\frac{d\rho I}{dt} & = \rho(\frac{\kappa}{qq} E-\frac{1}{qq}I)\\
			\frac{d\rho R}{dt} + \nabla[-\rho D \nabla R] & = \rho(\frac{(1-\kappa)}{qq} E+\frac{(1-\theta)}{pp}I)\\
			\frac{d\rho D}{dt} & = \rho \frac{\theta}{pp} I\\
		\end{split}
	\end{equation*}	
\end{frame}
%----------------------------------------------------------------------------------------
