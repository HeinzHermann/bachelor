...
\begin{enumerate}[label=$\bullet$]
	...
	\item \B{Infected (I)}: Individuals that are currently infected with the disease.
	...
	\item \B{Removed (R)}: Individuals that  have either overcome their infection and are now immune or have succumb to the disease.
...
Members of the Susceptible group (\B{S}) can transition to the Infected group (\B{I}). This transition rate is expressed in the
...
Transition also occurs from the \B{I} group to the removed group (\B{R}). This change is expressed by the variable 
...
A model like this expresses the dynamics of an epidemic in much more detail. While the SIR model does distinct between important
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\State Set i=0;
		\While {\{$\epsilon^i$ != 0 \B{or} (breaking conditions not fulfilled)\}}
