...
A simplified version of the \I{Gauss-Newton algorithm} is listed below as pseudo-code.
...
\label{sec:PSO}
Like the Gauss-Newton algorithm, \I{Particle Swarm Optimization} (PSO) is a method used to iteratively optimize a problem.
...
given iteration. As a trade off, the problem function does not need to be differentiable like it is the case for the \I{Gauss-Newton algorithm}. \newline

During the initial steps of PSO a problem space is defined, as well as a cost function $L$. $L$ takes a number of $n$ input values in the from of a
...
\textcolor{red}{how to make pseudocode better/easier to read?}
